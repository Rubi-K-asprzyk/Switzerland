...
{ # !! Initialisation
  ...
  liverData <- left_join(liverData,bryoEnv, by = join_by(Site_VDP, Site_Suisse, x, y)) %>% relocate(z,.after = y)
  mossData <- left_join(mossData,bryoEnv, by = join_by(Site_VDP, Site_Suisse, x, y)) %>% relocate(z,.after = y)

  # - Occ_Data: A community matrix of occurence with plot in rows and species in column (with of without metadata associated).
  Occ_Data <- list(MOSS = mossData, LIVER = liverData)
  # - Sp_Names: A vector of species names. 
  Sp_Names <- list(MOSS = Moss_Names, LIVER = Liver_Names)
  # - Phylo_Tree : Phylogenetic Tree associated with the dataset. 
  Phylo_Tree <- list(MOSS = Mosses_Tree, LIVER = Liver_Tree)
  # - Phylo_Tree_Null : Null Phylogenetic Tree associated with the dataset.  
  Phylo_Tree_Null <- list(MOSS = Mosses_Tree_Null, LIVER = Liver_Tree_Null)

  # Message
  cat(rule(left = "- Data Loaded. - ", line_col = "white", line = " ", col = "green"))
  cat(rule(left = "DONE: INITIALISATION", line_col = "green", line = "-", col = "br_green"))
...
# Choose the variables to use throughout the analyses. 

  # - Nrep: The number of NullModel Replicates to do.
  Nrep <- 3
  # - Threshold: The minimum number of species wanted in each plot. (Default = 2).
  Threshold <- list(MOSS = 4, LIVER = 3) # Mosses only accept a threshold of 4
  ...
  N_stripes <- 6
  # - Weighted: If {Alt_limits == NULL}, Are the stripes weighted by the number of plots (i.e with varying size) or not (i.e varying in the number of plots).
  Weighted <- TRUE
...
{ # !! Threshold filtering
  ...
  for (i in c("MOSS", "LIVER")) {
    # Computation of the SR and filtering of the data based on the SR Threshold
    Occ_Data[[i]] <- Occ_Data[[i]] %>%
      # Compute the Species Richness
      mutate(SR = apply(Occ_Data[[i]][, Sp_Names[[i]]], 1, sum), .after = y) %>%
      # Select the plots based on the Species richness threshold.
      dplyr::filter(SR > Threshold[[i]])
  }

# Message
cat(rule(left = "- Threshold filtered. - ", line_col = "white", line = " ", col = "green"))

} # !! End of Threshold filtering
...
{ # !! Range Choice
  ...
  # We only need it for the Mosses (for now) because they are the only ones we wan use in Step 2 and Step 3
  ...
  for (i in c("MOSS")) {
    ...
    if (is.null(Alt_limits) == FALSE) {
      ...
      Range_Number <- foreach(Z = Occ_Data[[i]]$z, .combine = c) %do% {
        # Apply the function
        Interval.finder(Z = Z)
      }
      ...
      # Create the ranges
      Occ_Data[[i]] <- mutate(Occ_Data[[i]],
        # Add the desired Range_Intervals
        Range_Intervals = case_when(Weighted == TRUE ~ cut_number(Occ_Data[[i]]$z, n = N_stripes, dig.lab = 4, right = FALSE), # Right = FALSE is used to have all breaks that start with a "[" to help for the correct ordering of the levels of the factor.
          Weighted == FALSE ~ cut(Occ_Data[[i]]$z, N_stripes, dig.lab = 4, right = FALSE),
          .default = "OSKOUR"
        ),
        # Add the desired Range_Number
        Range_Number = case_when(Weighted == TRUE ~ as.character(cut_number(Occ_Data[[i]]$z, n = N_stripes, dig.lab = 4, labels = F, right = FALSE)),
          Weighted == FALSE ~ as.character(cut(Occ_Data[[i]]$z, N_stripes, dig.lab = 4, labels = F, right = FALSE)),
          .default = "OSKOUR"
        ),
        .after = z
      )
...
#--------------------------------#
##### STEP 1: GLOBAL DATASET #####
#---------- ---------------------#

# Analyse the patterns of taxonomic and phylogenetic beta-diversity across all the altitudinal gradient. 

# Computed Metrics:
  
  # -- Taxonomic -- #
    # - Turnover (beta.sim) - Simpson PairWise dissimilarity.
    # - Nestedness (beta.sne) - Nestedness fraction of Sorensens PairWise dissimilarity.
    # - Total (beta.sor) - Sorensen PairWise dissimilarity. 
...
{ # !! STEP 1: GLOBAL DATASET
  ...
  cat(rule(left = "STEP 1 | GLOBAL DATASET.", line_col = "green", line = "-", col = "br_green"))

  cat(rule(left = "BEGIN: METRIC COMPUTATION", line_col = "green", line = "-", col = "br_green"))

  { # !! Taxonomic beta-Metrics computation

    Taxo.Beta <- foreach(i = c("MOSS", "LIVER")) %dopar%
      {
        # Add the rowname as a column to keep this info
        Beta.Data <- Occ_Data[[i]] %>%
        ...
        MetaData <- Beta.Data %>%
          ...
          dplyr::select(!any_of(Sp_Names[[i]]))
        ...
        Taxo.Beta <-
          ...
          pivot_longer(cols = c(beta.sim,beta.sne,beta.sor), values_to = "Value", names_to = "Metric")

      } %>% setNames(c("MOSS", "LIVER"))
  ...
  { # !! Phylogenetic Beta Metric Computation
    ...
    Phylo.Beta <- foreach(i = c("MOSS", "LIVER")) %dopar% {
      ##### ----- DATA PREPARATION ----- #####
      ...
      MetaData <- Occ_Data[[i]] %>%
        ...
        mutate_at("Plot", as.numeric) %>%
        # Select only the Metadata (based on the species names )
        dplyr::select(!any_of(c(Sp_Names[[i]])))
      ...
      Hardy_Metrics <- spacodiR::spacodi.calc(
        ...
        phy = Phylo_Tree[[i]], # phy a phylogenetic tree of class phylo or evolutionary distance matrix between species (see cophenetic.phylo)                   # sp.traits a species-by-trait(s) dataframe or a species traits distance matrix (see dist)
      ...
      # --- Combine the metrics altogether and pivot longer the metrics --- #
      ...
      # Message
      cat(rule(left = paste0("- BEGIN: Computation of ",i," Phylo-Beta Metrics NULL MODEL - "), line_col = "#3C917E", line = " ", col = "#3C917E"))

      # /!\ .combine uses a custom function with "x" the previous iteration and "y" the new iteration. It is used to give the "by" argument without display message and clutter the console. function(x, y) {full_join(x, y, by = join_by(Sample, PlotA, PlotB, Metric, Value, Type))}

      Phylo.beta.Null <- foreach(r = 1:Nrep, .combine = ) %dopar% {
        # -- Compute the PIst metrics -- #

        Hardy_Metrics <- spacodiR::spacodi.calc(
          sp.plot = Beta.Data, # sp.plot =  a community dataset in spacodiR format (see as.spacodi) i.e species in rows and plots in columns
          phy = Phylo_Tree_Null[[i]][[r]], # phy a phylogenetic tree of class phylo or evolutionary distance matrix between species (see cophenetic.phylo)                   # sp.traits a species-by-trait(s) dataframe or a species traits distance matrix (see dist)
          all.together = TRUE, # whether to treat all traits together or separately
          prune = TRUE,
          pairwise = TRUE
        )
        ...
        # Print a message
        cat(rule(left = paste0("- Rep ", r, " / ", Nrep, " - "), line_col = "#73913C", line = " ", col = "#73913C"))
      ...
      # Message
      cat(rule(left = paste0("- END: Computation of ",i," Phylo-Beta Metrics NULL MODEL - "), line_col = "#3C917E", line = " ", col = "#3C917E"))

      ##### ----- OBSERVED + NULL MODEL COMBINATION ----- #####

      # Combine all Null Models together.
      # Phylo.beta <- purrr::reduce(Phylo.beta.Null,rbind)

      # Combine observed and Null models together.
      # Phylo.beta <- full_join(Phylo.beta.Obs,Phylo.beta,by = join_by(Sample, PlotA, PlotB, Metric, Value, Type))

      # ------------- #

      return(list(OBS = Phylo.beta.Obs, Null = Phylo.beta.Null))
    } %>% setNames(c("MOSS","LIVER"))

  # Combine the taxonomic and phylogenetic beta results


  Beta.results.Obs <- foreach(i = c("MOSS", "LIVER")) %dopar% {
  
  full_join(Phylo.Beta[[i]]$OBS,Taxo.Beta[[i]],by = join_by(Sample, PlotA, PlotB, Metric, Value)) %>%
  mutate_at(c("PlotA", "PlotB"), as.numeric) %>%
  # Join the metadata for both plots used in the pairwise computation.
  left_join(MetaData, c("PlotA" = "Plot")) %>% # Plot A
  left_join(MetaData, c("PlotB" = "Plot"), suffix = c("_A", "_B")) %>% # Plot B + add suffixes "A" and "B" to distinguish the two plots.
  ...
  cat(rule(left = "END: METRIC COMPUTATION", line_col = "green", line = "-", col = "br_green"))

} # !! END OF STEP 1 | GLOBAL DATA
