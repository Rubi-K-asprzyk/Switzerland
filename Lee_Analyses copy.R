...
# Load the phylotrees
Liver_Tree <- read.tree("PhyloTree/PhyloTree/timetree50mod-liverwortsV2.nwk")
Mosses_Tree <- read.tree("PhyloTree/PhyloTree/timetree50mod-mossesV2.nwk")
...
SR.liver <- apply(liverData[,5:ncol(liverData)],1,sum)
SR.moss <- apply(mossData[,5:ncol(mossData)],1,sum)

# PD
PD.liver <- pd(liverData[,5:ncol(liverData)], Liver_Tree, include.root=F)$PD
PD.moss <- pd(mossData[,5:ncol(mossData)], Mosses_Tree, include.root=F)$PD

# MPD
  # Compute the cophenetic distance matrix
  dis.liver <- cophenetic(Liver_Tree)
  dis.moss <- cophenetic(Mosses_Tree)
  
  # Create a new smaller community matrix for the computation of MPD that contains only the species present in the tree
  data.mpd.liver <- select(liverData,any_of(Liver_Tree$tip.label))
  data.mpd.moss <- select(mossData,any_of(Mosses_Tree$tip.label))
  
  # Reorder it to be the same as in the community matrix 
  dis.liver <- dis.liver[colnames(data.mpd.liver),colnames(data.mpd.liver)]
  dis.moss <- dis.moss[colnames(data.mpd.moss),colnames(data.mpd.moss)]

MPD.liver <- mpd(samp = data.mpd.liver, dis = dis.liver, abundance.weighted	= F)
MPD.moss <-  mpd(samp = data.mpd.moss, dis = dis.moss, abundance.weighted	= F)

# Combine all the results
results.bryo <- data.frame(SR.bryo,SR.liver,SR.moss,PD.liver,PD.moss,MPD.liver,MPD.moss)
...
TracheoMeta <- TracheoData[,1:2]       # Verification: table(as.matrix(TracheoOcc))
TracheoData <- TracheoData[,-c(1:2)]

# Get the species names
Sp_names <- colnames(TracheoData)
...
# Get the species names
colnames(TracheoData) <- Sp_names

# Trimm the phylotree with the species present in the community dataset
tracheo.Tree <- get_subtree_with_tips(ZanneTree,only_tips = Sp_names)$subtree
...
# PD
PD.tracheo <- pd(TracheoData, tracheo.Tree, include.root=F)$PD

# MPD
  # Compute the cophenetic distance matrix
  dis.tracheo <- cophenetic(tracheo.Tree)

  # Create a new smaller community matrix for the computation of MPD that contains only the species present in the tree
  data.mpd.tracheo <- select(TracheoData,any_of(tracheo.Tree$tip.label))

  # Reorder it to be the same as in the community matrix 
  dis.tracheo <- dis.tracheo[colnames(data.mpd.tracheo),colnames(data.mpd.tracheo)]

  
MPD.tracheo <- mpd(samp = data.mpd.tracheo, dis = dis.tracheo, abundance.weighted	= F)

# Combine all the results
results.bryo <- cbind(results.bryo,SR.tracheo,PD.tracheo,MPD.tracheo)

#-----------------------------#
##### CORRELATIONS + GLMs #####
#-----------------------------#

##### . Correlations ~ altitude . #####

  # Species Richness
cor(SR.bryo,alti) #0.197
cor(SR.liver,alti) #0.260
cor(SR.moss,alti) #0.164
cor(SR.tracheo,alti) #-0.409

  # PD
cor(PD.liver,alti, use = "complete.obs") #0.18
cor(PD.moss,alti,  use = "complete.obs") #0.221
cor(PD.tracheo,alti,  use = "complete.obs") #-0.288

  # MPD
cor(MPD.liver,alti, use = "complete.obs") # -0.288
cor(MPD.moss,alti,  use = "complete.obs") # 0.336
cor(MPD.tracheo,alti,  use = "complete.obs") # 0.081

##### . GLM . #####

  # Species Richness
SR.bryo.glm <- glm(SR.bryo~alti+I(alti^2),family = poisson())
  with(summary(SR.bryo.glm), 1 - deviance/null.deviance) # Explained deviance = 0.143
SR.liver.glm <- glm(SR.liver~alti+I(alti^2),family = poisson())
  with(summary(SR.liver.glm), 1 - deviance/null.deviance) # Explained deviance = 0.248
SR.moss.glm <- glm(SR.moss~alti+I(alti^2),family = poisson())
  with(summary(SR.moss.glm), 1 - deviance/null.deviance) # Explained deviance = 0.117
SR.tracheo.glm <- glm(SR.tracheo~alti+I(alti^2),family = poisson())
  with(summary(SR.tracheo.glm), 1 - deviance/null.deviance) # Explained deviance = 0.254

  # PD
PD.liver.glm <- glm(PD.liver~alti+I(alti^2),family = poisson())
  with(summary(PD.liver.glm), 1 - deviance/null.deviance) # Explained deviance = 0.196
PD.moss.glm <- glm(PD.moss~alti+I(alti^2),family = poisson())
  with(summary(PD.moss.glm), 1 - deviance/null.deviance) # Explained deviance = 0.084
PD.tracheo.glm <- glm(PD.tracheo~alti+I(alti^2),family = poisson())
  with(summary(PD.tracheo.glm), 1 - deviance/null.deviance) # Explained deviance = 0.206
  
  # MPD
MPD.liver.glm <- glm(MPD.liver~alti+I(alti^2),family = poisson())
  with(summary(MPD.liver.glm), 1 - deviance/null.deviance) # Explained deviance = 0.086
MPD.moss.glm <- glm(MPD.moss~alti+I(alti^2),family = poisson())
  with(summary(MPD.moss.glm), 1 - deviance/null.deviance) # Explained deviance = 0.127
MPD.tracheo.glm <- glm(MPD.tracheo~alti+I(alti^2),family = poisson())
  with(summary(MPD.tracheo.glm), 1 - deviance/null.deviance) # Explained deviance = 0.061

# Can do summary() on all of these tests. 
  
  ##### . Prediction . #####

# Prepare a dataset of prediction for the altitude
alti.pred <- as.data.frame(c(min(alti):max(alti)))
colnames(alti.pred)="alti"

# Create a function to compute the prediction on all the datasets.
prediction <- function(data, metric, taxa){
  # Compute the prediction across the altitudinal range
  data.pred <- predict(data, newdata = alti.pred, type="response",se.fit=T)
  # Extract the standard deviation
  data.pred.min <- data.pred$fit - data.pred$se.fit
  data.pred.max  <- data.pred$fit + data.pred$se.fit
  # Extract the fit
  data.pred <- data.pred$fit
  # return the results
  result <- data.frame(metric = metric, "taxa" = taxa, data.pred = data.pred, alti.pred, mini = data.pred.min, maxi = data.pred.max)
  return(result)
...
# Compute the predictions
  ...
  # Species Richness
predict.SR.bryo  <- prediction(data = SR.bryo.glm, metric = "SR", taxa = "bryo")
predict.SR.liver <- prediction(data = SR.liver.glm, metric = "SR", taxa = "liver")
predict.SR.moss <- prediction(data = SR.moss.glm, metric = "SR", taxa = "moss")
predict.SR.tracheo <- prediction(data = SR.tracheo.glm, metric = "SR", taxa = "tracheo")

  # PD
predict.PD.liver <- prediction(data = PD.liver.glm, metric = "PD", taxa = "liver")
predict.PD.moss <- prediction(data = PD.moss.glm, metric = "PD", taxa = "moss")
predict.PD.tracheo <- prediction(data = PD.tracheo.glm, metric = "PD", taxa = "tracheo")

  # PD
predict.MPD.liver <- prediction(data = MPD.liver.glm, metric = "MPD", taxa = "liver")
predict.MPD.moss <- prediction(data = MPD.moss.glm, metric = "MPD", taxa = "moss")
predict.MPD.tracheo <- prediction(data = MPD.tracheo.glm, metric = "MPD", taxa = "tracheo")

# Find the altitude where the metric is max with this piece of code
alti.pred[which.max(predict.SR.bryo$data.pred),1] # 1814.844m

# Create a global dataframe
results.predict <- join_all(list(predict.SR.bryo,predict.SR.liver,predict.SR.moss,predict.SR.tracheo,predict.PD.liver,predict.PD.moss,predict.PD.tracheo,predict.MPD.liver,predict.MPD.moss,predict.MPD.tracheo),
                            type = "full")

##### . Plotting . #####

results.predict %>%
  ggplot(aes(x = alti,y = data.pred, fill=taxa, color=taxa, ymin = mini, ymax = maxi)) +
  # Draw the line
  geom_line() +
  # Draw the interval of confidence
  geom_ribbon(alpha= 0.2,linetype="dotted",linewidth=0.5) +
  # Split between metrics 
  facet_wrap(. ~ metric, scales = "free_y") +
  # Labels
  ylab("Predicted metric values")+ xlab("Elevation (m)")
...
bryo.nbg <- graph2nb(gabrielneigh(bryoData[,c("x","y")]), sym = TRUE) # Spatial links
bryo.listw <- nb2listw(bryo.nbg)

# Compute the LEEs index

  # -- Species richness -- # 

  # Lee
lee.SR.bryo <- lee(SR.bryo,SR.tracheo,bryo.listw,n=413) # Global = 0.1275
lee.mc.SR.bryo <- lee.mc(SR.bryo,SR.tracheo,bryo.listw,nsim=1000, alternative="greater",spChk=T)

  # glm
lee.glm.SR <- glm(lee.SR.bryo$localL~alti+I(alti^2))
with(summary(test), 1 - deviance/null.deviance) # Explained deviance = 0.269
  
  # Predict
lee.predict.SR.bryo <- predict(lee.glm.SR, newdata = alti.pred,type="response")
  
  # Plot
plot(lee.predict.SR.bryo~alti.pred[,1],type="l",xlab="Elevation (m)", ylab="Lee's L")
points(lee.SR.bryo$localL~alti)

# -- PD -- # 

# Create neighbor lists 
liver.nbg <- graph2nb(gabrielneigh(liverData[,c("x","y")]), sym = TRUE) # Spatial links
liver.listw <- nb2listw(liver.nbg)

# Lee
lee.PD.liver <- lee(PD.liver, PD.tracheo, liver.listw, n=413, na.action = "na.omit") # Global = 0.1275
lee.mc.PD.liver <- lee.mc(PD.liver, PD.tracheo, liver.listw, nsim=1000, alternative="greater",spChk=T, na.action = "na.omit")
